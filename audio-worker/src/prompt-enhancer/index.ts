 ...
 * 
 * Orchestrator for naming convention generation.
 * 
 ...
 * 
 * 2. Naming Convention Generation → This service (generateNaming)
 *    - Called by worker during job processing
 *    - Generates UCS-compliant filenames from prompt metadata
 *    - Can be skipped if user disables naming (createSimpleResult)
 * 
 * Files:
 * - types.ts: Shared types and schemas
 * - builders/naming-only.ts: Mode-specific naming prompts (classification only)
...
// Naming-only builders (classification without prompt modification)
import {
    getArtistPrefix,
 ...
 * Prompt enhancement is now handled by Edge Function.
 */
export class PromptEnhancerService {
    ...
    /**
     * Build naming-only prompt based on mode
     * Used to generate metadata for filename (no prompt enhancement)
     */
    private buildNamingPrompt(
        ...
        switch (mode) {
            case 'creator':
                return buildCreatorNamingPrompt(userPrompt, config);
            case 'producer':
                return buildProducerNamingPrompt(userPrompt, config);
            case 'designer':
            default:
                return buildDesignerNamingPrompt(userPrompt, config, ucsContext);
        }
    }

    /**
     * Generate naming convention metadata (no prompt enhancement)
     * 
     * Called during job processing to generate UCS-compliant filenames.
     * The prompt text is NOT modified - only metadata is extracted for naming.
     * 
    ...
    async generateNaming(
        ...
        try {
            ...
            // Build naming-only prompt (focused on classification)
            const prompt = this.buildNamingPrompt(userPrompt, config, mode, ucsContext);
            ...
            const metadata: PromptMetadata = {
                ...namingResponse,
                enhanced_prompt: userPrompt, // Pass through original/enhanced prompt
            ...
            const result: EnhancedPromptData = {
                ...
                enhancedPrompt: userPrompt, // Prompt unchanged by this service
                filename,
                metadata,
            };
    ...
    createSimpleResult(
        ...
        const simpleMetadata: PromptMetadata = {
            enhanced_prompt: userPrompt,
            catId: 'GEN',
            category: 'GEN',
            subcategory: '',
